...
dat_wchars$traveltime <- raster::extract(access.diag, dat_wchars)
dat_wchars$traveltime_t <- raster::extract(access.trt, dat_wchars)
...
ggplot(dat_wchars, aes(traveltime_t)) + 
  geom_histogram(bins = 50) +
  labs(title = "Travel time to most accessible diagnosis facility",
       x = "Time (minutes)",
       y = "Count")
